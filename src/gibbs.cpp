...
static void gibbstemps(Work& W,const Data& Dat,const Param& P,const Prior& Pr,int ncogn,Rng& rng){
  ...
  int nch=P.nch;
  std::vector<std::vector<Mat>> loiappc(nch);
  for(int x=0;x<nch;++x){ loiappc[x].resize(trc.n_edges());
  ...
  for(int x=0;x<nch;++x)
    lint[x]=pruning_full(trc,W.X[x],W.M[x],W.L,Dat[x],P.nph[x],trc.root_nodes,P.loiini[x],loiappc[x]);
  ...
  if(std::log(rng.runif())<acc){ W.tr=trc; W.lin=lint; W.loiapp=loiappc; }
}
